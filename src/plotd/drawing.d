...
{
    auto surface = new cairo.ImageSurface(cairo.Format.CAIRO_FORMAT_ARGB32, width,
        height);
...
cairo.Surface createPlotSurfacePDF(string name, int width = 400, int height = 400)
{
    static if (cconfig.CAIRO_HAS_PDF_SURFACE)
    {
        cairo.Surface surface = new cpdf.PDFSurface(name, width, height);
        auto context = cairo.Context(surface);
        clearContext(context);
        return surface;
    }
...
cairo.Surface createPlotSurfaceSVG(string name, int width = 400, int height = 400)
{
    static if (cconfig.CAIRO_HAS_SVG_SURFACE)
    {
        cairo.Surface surface = new csvg.SVGSurface(name, width, height);
        auto context = cairo.Context(surface);
        clearContext(context);
        return surface;
    }
...
CONTEXT drawPoint(CONTEXT)(const Point point, CONTEXT context)
{
    ...
    context.rectangle(point.x - width_height.x / 2.0, point.y - width_height.y / 2.0,
...
unittest
{
    ...
    auto mock = mocker.mockStruct!(cairo.Context, cairo.Surface)(surface);
...
CONTEXT drawAxes(CONTEXT)(const Bounds bounds, CONTEXT context)
{
    ...
    // Draw xaxis
    context = drawLine(Point(xaxis.min, yaxis.min), Point(xaxis.max, yaxis.min),
        context);
    // Draw ticks
    ...
    // Draw yaxis
    context = drawLine(Point(xaxis.min, yaxis.min), Point(xaxis.min, yaxis.max),
        context);
    // Draw ticks
    ...
    {
        context = drawLine(Point(xaxis.min, tick_y), Point(xaxis.min + tick_size,
            tick_y), context);
        context.save;
        context.identityMatrix;
        auto extents = context.textExtents(tick_y.to!string);
        auto textSize = cairo.Point!double(extents.height, -0.5 * extents.width);
        context.restore;
        textSize = context.deviceToUserDistance(textSize);
        context = drawRotatedText(tick_y.to!string, Point(xaxis.min - 0.5 * textSize
            .x, tick_y - textSize.y), 1.5 * 3.14, context);
...
CONTEXT drawXLabel(CONTEXT)(string label, Bounds bounds, CONTEXT context)
{
    ...
    auto textSize = cairo.Point!double(0.5 * extents.width, -extents.height);
    textSize = context.deviceToUserDistance(textSize);
    context = drawText(label, Point(bounds.min_x + bounds.width / 2.0 - textSize.x,
        bounds.min_y - 3.0 * textSize.y), context);
    return context;
}
...
CONTEXT drawYLabel(CONTEXT)(string label, Bounds bounds, CONTEXT context)
{
    ...
    auto textSize = cairo.Point!double(-extents.height, 0.5 * extents.width);
    textSize = context.deviceToUserDistance(textSize);
    context = drawRotatedText(label, Point(bounds.min_x + 2.0 * textSize.x, bounds
        .min_y + bounds.height / 2.0 + textSize.y), 1.5 * 3.14, context);
    return context;
}
...
CONTEXT drawRotatedText(CONTEXT)(string text, const Point location, double radians,
    CONTEXT context)
{
    context.moveTo(location.x, location.y);
    context.save();
...
CONTEXT drawBins(T : Bins!size_t, CONTEXT)(CONTEXT context, Bins!T bins)
{
    ...
    foreach (x, ybins; bins)
    {
        foreach (y, value; ybins)
        {
            ...
            context.rectangle(x, y, bins.width, ybins.width);
            context.fill();
        }
    }
    return context;
...
CONTEXT drawBins(T : size_t, CONTEXT)(CONTEXT context, Bins!T bins)
{
    ...
    {
        context = drawLine(Point(x, 0), Point(x, count.to!double), context);
        context = drawLine(Point(x, count.to!double), Point(x + bins.width, count
            .to!double), context);
